...
const TAG = "[tools/bundle-open-api-tpl-files.ts]";
...
async function createRequest(
  ...
  const parsedCfg = await yargs(hideBin(argv))
    ...
    .option("verbose", {
      alias: "v",
      type: "boolean",
      description: "Whether logs should be sent to stdout/stderr or not.",
      default: false,
    })
...
async function bundleOpenApiTplFilesOneFile(
  ...
  log(req, "%s templateDirPath=%s", TAG, templateDirPath);

  const outputPathAbsolute = path.join(templateDirPath, "openapi.json");
  log(req, "%s outputPathAbsolute=%s", TAG, outputPathAbsolute);

  log(req, "%s Bundling %s ...", TAG, templatePathAbsolute);
  ...
  log(req, "%s Bundled %s OK", TAG, templatePathAbsolute);
  if (!openApiJson.info) {
  ...
  log(req, `${TAG} writing changes to disk or ${outputPathAbsolute}`);
...
export async function bundleOpenApiTplFiles(
  ...
  log(req, `${TAG} Globbing openapi.tpl.json files %o ${specTemplateGlobs}`);
  log(req, `${TAG} Detected ${oasPaths.length} openapi.tpl.json spec files`);
  log(req, `${TAG} Git diff check disabled: ${req.checkGitDiffDisabled}`);
  log(req, `${TAG} File paths found:`, JSON.stringify(oasPaths, null, 4));
  ...
  log(req, `${TAG} Saving final report to: ${reportPath}`);
