...
All paths in following steps are relative to this folder (`examples/weaver-ledger-sync/network-setups/fabric/dev`).
...
2. Go to `examples/weaver-apps/fabric/fabric-cli`, and follow these steps:
...
4. Go to `examples/weaver-apps/fabric/fabric-cli`, and run following commands to create credentials for fabric networks:
    ```
...
5. Now update credentials in other directories:
    - Go to `examples/weaver-apps/fabric/fabric-cli` and copy `src/data/credentials/network1` and `src/data/credentials/network2` to `src/data/credentials_docker` directory.
    - Go to `examples/weaver-apps/corda/corda-simple-application/clients/src/main/resources/config` and then follow these steps:
        1. With reference from `examples/weaver-apps/fabric/fabric-cli/src/data/credentials/network1/access-control.json`, update the `rules.principal` key in `network1/access-control.json`.
        2. Copy `examples/weaver-apps/fabric/fabric-cli/src/data/credentials/network1/membership.json` to `credentials/network1/membership.json`.
        3. Repeat steps `a` and `b` for `network1-docker`.
        4. Repeat steps `a` and `b` for `network2` by replacing `network1` with `network2`.
6. Go to `examples/weaver-ledger-sync/network-setups/fabric/dev` folder, and finally run, to create backup of artifacts:
    ```
    make backup
    ```
