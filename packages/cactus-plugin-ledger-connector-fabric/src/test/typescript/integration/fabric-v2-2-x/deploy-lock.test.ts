...
import { AssetLockUtils } from "../../fixtures/go/lock/chaincode-typescript/src/util";
import { DiscoveryOptions } from "fabric-network";
import { PluginKeychainMemory } from "@hyperledger/cactus-plugin-keychain-memory";
import { Configuration } from "@hyperledger/cactus-core-api";
import { Asset } from "../../fixtures/go/lock/chaincode-typescript/src/asset";
...
test(testCase, async (t: Test) => {
  ...
  const fnTag = `deploy lock test`;

  const log = LoggerProvider.getOrCreate({
    label: fnTag,
    level: "INFO",
  });
  log.warn("starting ledger");
  ...
  const contractName = "lock-asset-1";

  const contractRelPath = "../../fixtures/go/lock/chaincode-typescript";
  ...
  const assetLockUtil = new AssetLockUtils();
  const { privateKey, publicKey } = Secp256k1Keys.generateKeyPairsBuffer();
  const privKeyStr = assetLockUtil.bufArray2HexStr(privateKey);
  const ID = assetLockUtil.bufArray2HexStr(publicKey);
  ...
  t.comment(`LockAsset.Create(): ${JSON.stringify(createRes.data)}`);
  ...
  t.comment(`LockAsset.get() ResponseBody: ${JSON.stringify(getRes.data)}`);

  let asset0: Asset = JSON.parse(getRes.data.functionOutput);

  t.ok(asset0, "JSON.parse(getRes.data.functionOutput) truthy OK");

  t.ok(asset0.ID, "asset.ID truthy OK");
  t.equal(asset0.ID, ID, "asset.ID === assetId truthy OK");
  ...
  t.ok(asset0.Nonce, "asset.Nonce truthy OK");
  t.equal(asset0.Nonce, 0, "asset.Nonce === 0");
  t.ok(asset0.IsLock, "asset.IsLock truthy OK");
  t.equal(asset0.IsLock, false, "asset.IsLock === false");

  asset0.Nonce++;
  const lockSignature = await assetLockUtil.sign(
    JSON.stringify(asset0),
    privKeyStr,
  );
  const lockRes = await apiClient.runTransactionV1({
    contractName,
    channelName,
    params: [ID, lockSignature],
  ...
  t.ok(lockRes, "getRes truthy OK");
  t.ok(lockRes.data, "getRes.data truthy OK");
  t.true(
    lockRes.status > 199 && createRes.status < 300,
    "getRes status 2xx OK",
  );
  t.comment(`LockAsset.lock() ResponseBody: ${JSON.stringify(getRes.data)}`);

  const getRes1 = await apiClient.runTransactionV1({
    contractName,
    channelName,
    params: [ID],
    methodName: "ReadAsset",
  ...
  t.ok(getRes1, "getRes truthy OK");
  t.ok(getRes1.data, "getRes.data truthy OK");
  t.ok(getRes1.data.functionOutput, "getRes.data.functionOutput truthy OK");
  t.true(getRes1.status > 199 && lockRes.status < 300, "getRes status 2xx OK");
  t.comment(`LockAsset.get() ResponseBody: ${JSON.stringify(getRes.data)}`);

  asset0 = JSON.parse(getRes1.data.functionOutput);
  ...
  // repo has the spelling different from the golang chaincode as well.
  t.ok(asset0.Nonce, "asset.Nonce truthy OK");
  t.equal(asset0.Nonce, 1, "asset.Nonce === 0");
  t.ok(asset0.IsLock, "asset.IsLock truthy OK");
  t.equal(asset0.IsLock, true, "asset.IsLock === false");
